...
function main(){
  ...
  if (typeof attendants == 'undefined') {
    alert("Error: attendants variable is not set yet."); return;
  }
...
// fix for undefined Occur
function undefinedFix(){
  winnerList = localStorage.winners.split("|");
  localStorage.setItem('winners', '');
  $('.winner li').removeClass('list-group-item-active');
  for (var i = 0; i < winnerList.length; i++) {
    // removing undefined
    if( winnerList[i].match(/^undefined|undefined\:[\w]+$/) ) return;
    winnerList[i] = winnerList[i].split(":");
    winnerList[i] = { name: winnerList[i][0], gift: winnerList[i][1] };
    localStorage.setItem("winners", ((localStorage.winners&&localStorage.winners+"|")||"")+winnerList[i].name+":"+winnerList[i].gift);
    attendants.splice(attendants.indexOf(winnerList[i].name), 1);
    var liItem = '<li class="text-center clearfix text-uppercase list-group-item '+currentGift.abbr+'">' +
      '<span class="badge badge-danger pull-left">' + i + '</span>' + winnerList[i].name +
      '<span class="'+
      (function(name){
        var rs; Array.prototype.forEach.call(gifts, function(gift){if(gift.name === name)rs=gift;return false;});
        return rs.icon;
      }(winnerList[i].gift))+
      ' pull-right"></span></li>';
    $winnerListEl.append(liItem);
